...
bool SpiDevice::Init() {
  ...
  for (auto& [name, actr] : actuator_map_) {
    ...
    // Initialize per-motor queue
    uint8_t motor_id = actr->GetId();
    motor_queues_[motor_id] = std::deque<CanFrame>();
    motor_order_.push_back(motor_id);
    
    LOG_DEBUG("Actuator %s (Motor ID %u) initialized with dedicated queue", 
              name.c_str(), motor_id);
  }

  // Sort motor order for consistent round-robin
  std::sort(motor_order_.begin(), motor_order_.end());
  
  LOG_INFO("SpiDevice %s initialized with %zu actuators in round-robin order", 
...
void SpiDevice::OnDataReceived(uint32_t can_id, const uint8_t* data) {
  // Extract motor ID from CAN ID (bits 8-15 as you confirmed)
  ...
  if (it != response_map_.end()) {
    ...
    LOG_DEBUG("Unknown motor ID: 0x%02X (CAN ID: 0x%08X)", motor_id, can_id);
  }
}
...
// Ensures all motors get equal transmission opportunities
// ============================================================
void SpiDevice::QueueCommand(uint32_t can_id, const uint8_t* data, uint8_t motor_id) {
  if (data == nullptr) {
  ...
  // Add to motor-specific queue
  motor_queues_[motor_id].push_back(frame);
  
  LOG_DEBUG("Command queued for Motor ID %u: CAN ID 0x%08X, queue depth: %zu", 
            motor_id, can_id, motor_queues_[motor_id].size());
}

bool SpiDevice::GetNextTxData(uint32_t& can_id, uint8_t* data) {
  std::lock_guard<std::mutex> lock(queue_mtx_);
  
  if (motor_order_.empty()) {
    return false;
  }
  
  // ============================================================
  // KEY FIX: Round-robin across motors
  // Try each motor in order until we find one with data
  // ============================================================
  size_t attempts = 0;
  while (attempts < motor_order_.size()) {
    uint8_t motor_id = motor_order_[next_motor_idx_];
    next_motor_idx_ = (next_motor_idx_ + 1) % motor_order_.size();
    
    auto& queue = motor_queues_[motor_id];
    if (!queue.empty()) {
      ...
      LOG_DEBUG("Sending Motor ID %u: CAN ID 0x%08X, remaining in queue: %zu",
                motor_id, can_id, queue.size());
...
bool SpiDevice::EnableActuator(const std::string& name) {
  ...
  // FIXED: Pass motor ID for round-robin queuing
  QueueCommand(actr->GetCanId(), buffers.send, actr->GetId());
  
  LOG_DEBUG("Actuator %s enable command queued (Motor ID: %u, CAN ID: 0x%08X)", 
            name.c_str(), actr->GetId(), actr->GetCanId());
  return true;
}
